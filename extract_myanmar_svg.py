...
a raster grid CSV aligned to PRIO-GRID cells.
...
def latlon_to_priogrid(lat, lon):
    """
    Convert lat/lon to PRIO-GRID cell ID and cell center coordinates.
    Grid originates at SW corner (90S, 180W), 0.5 degree cells.
    Formula verified against official PRIO-GRID examples.
    """
...
def get_myanmar_priogrid_cells():
    """
    Return all PRIO-GRID cells covering Myanmar's bounding box.
    Myanmar: roughly 92E-101.5E, 9.5N-28.5N
    """
    cells = []
    # Generate all cell centers in bounding box
    lons = np.arange(92.0, 102.0, 0.5)
    lats = np.arange(9.5,  29.0,  0.5)
    for lat in lats:
        for lon in lons:
            # Use cell center (add 0.25 to get center of cell starting at lat,lon)
            clat = lat + 0.25
            clon = lon + 0.25
            gid, lat_c, lon_c = latlon_to_priogrid(clat, clon)
...
def rasterize_priogrid(features, date_str):
    """
    Assign each PRIO-GRID cell inside Myanmar to a faction using majority vote.
    For each PRIO-GRID cell, sample multiple points within the cell and take
    the majority faction — more robust than single center point at 0.5 degree.
    """
    # Get all PRIO-GRID cells in Myanmar bounding box
    pg_cells = get_myanmar_priogrid_cells()

    # Build shapely geometries — non-Junta first, Junta last as background
    geoms_primary = []
    junta_geom = None
    ...
    for i, (gid, lat_c, lon_c) in enumerate(pg_cells):
        if i % 100 == 0:
            print(f"  Rasterizing... {i}/{n} cells", end='\r')

        # Sample 9 points within the cell (3x3 grid) for majority vote
        ...
        # Majority vote — exclude 'Outside Myanmar' if any real faction present
        real_votes = [v for v in votes if v != 'Outside Myanmar']
        if real_votes:
            faction = Counter(real_votes).most_common(1)[0][0]
        else:
            faction = 'Outside Myanmar'
