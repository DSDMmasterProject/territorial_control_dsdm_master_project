...
│   └── processed/              # Pipeline outputs (created locally, not committed)
│
├── models/
...
│
├── legacy/                     # Archived experimental notebooks and superseded scripts
...
The `data/` directory is **not included in the repository** — it is listed in
`.gitignore` to keep raw conflict datasets and large processed files out of version
control (ACLED in particular carries licensing terms that prevent redistribution).
After cloning, you must create the `data/` structure yourself and download the raw
files below. The `data/processed/` folder starts empty and is populated as you run
the pipeline.
...
- **`selected_features_32.csv`** (used by `feature_ablation.py` and
  ...
  `legacy/10_focal_mask_all_features.ipynb`. There is no standalone script
  in `src/` to reproduce it; regenerating it from scratch requires running
  that notebook manually. Since `data/` is not committed, this file must be
  regenerated (or supplied) locally before running the ablation.
...
- **`fetch_wikipedia_ground_truth.py`** makes ~30 Wikipedia API requests with
  ...
  internet connectivity. Because `data/` is not committed, you must run this
  script (or supply its output) when building the panel; re-running it is only
  necessary when extending the panel to new months.
