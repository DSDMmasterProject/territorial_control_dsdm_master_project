...
a PRIO-GRID aligned CSV.

Usage:
    python extract_myanmar_svg.py path/to/myanmar_YYYY-MM-DD.svg

Output:
    myanmar_YYYY-MM-DD.geojson
    myanmar_YYYY-MM-DD_grid.csv  columns: priogrid_gid, lat, lon, faction, date
...
GCPS = [
    (581.1, 1235.5, 16.866, 96.195), (577.1, 724.1, 21.975, 96.084),
    (578.6, 948.9, 19.745, 96.129),  (724.0, 1269.7, 16.490, 97.628),
    (699.3, 378.0, 25.383, 97.400),  (736.7, 623.0, 22.934, 97.752),
    (268.0, 895.5, 20.152, 92.900),  (466.4, 901.4, 20.149, 94.932),
    (447.6, 1238.6, 16.774, 94.732), (668.3, 840.0, 20.789, 97.037),
    (748.8, 518.5, 23.988, 97.657),  (776.8, 1508.0, 14.083, 98.196),
    (551.1, 827.4, 20.879, 95.859),  (485.0, 697.8, 22.108, 95.135),
    (685.3, 928.4, 19.674, 97.210),  (607.2, 1178.8, 17.335, 96.480),
    (512.6, 1032.0, 18.818, 95.218), (394.3, 599.0, 23.212, 94.014),
]

def latlon_to_priogrid(lat, lon):
    col = int(np.floor((lon + 180.0) / 0.5)) + 1
    row = int(np.floor((lat + 90.0)  / 0.5)) + 1
    col = max(1, min(720, col))
    row = max(1, min(360, row))
    gid = (row - 1) * 720 + col
    lon_center = (col - 0.5) * 0.5 - 180.0
    lat_center = (row - 0.5) * 0.5 - 90.0
    return gid, round(lat_center, 4), round(lon_center, 4)

def get_myanmar_priogrid_cells():
    cells = []
    for lat in np.arange(9.5, 29.0, 0.5):
        for lon in np.arange(92.0, 102.0, 0.5):
            gid, lat_c, lon_c = latlon_to_priogrid(lat + 0.25, lon + 0.25)
            cells.append((gid, lat_c, lon_c))
    return list({g[0]: g for g in cells}.values())
...
def _load_myanmar_border():
    ...
    if os.path.exists(border_path):
        ...
        return Polygon([
            [92.30,28.00],[92.60,28.30],[93.20,28.60],[94.00,28.50],[95.00,28.20],
            [96.00,28.40],[96.60,28.50],[97.00,28.30],[97.40,28.50],[97.80,28.20],
            [98.40,28.00],[98.70,27.50],[99.00,26.80],[99.40,26.20],[99.80,25.00],
            [100.20,23.80],[100.80,23.00],[101.20,22.50],[101.60,22.20],[101.50,21.50],
            [100.80,21.00],[100.30,20.50],[100.10,20.10],[99.50,19.00],[99.20,18.50],
            [99.00,17.50],[98.60,16.50],[98.80,15.50],[98.60,15.00],[98.30,14.00],
            [99.00,13.00],[99.20,11.50],[99.00,10.50],[98.50,10.00],[98.00,10.50],
            [97.50,11.00],[98.00,13.00],[97.80,15.00],[97.50,16.00],[97.00,17.00],
            [96.50,17.50],[96.00,18.00],[95.50,19.00],[94.50,19.50],[94.00,20.00],
            [93.50,20.50],[93.00,21.00],[92.80,21.50],[92.30,22.00],[92.10,23.00],
            [92.00,24.00],[92.20,25.00],[92.00,26.00],[92.10,27.00],[92.30,28.00],
        ])
...
def px_to_geo(points, lat_interp, lon_interp, c_lat, c_lon):
    pts = np.array(points)
    lats = lat_interp(pts); lons = lon_interp(pts)
    nm = np.isnan(lats)
    if nm.any():
        X2 = np.column_stack([pts[nm,0], pts[nm,1], np.ones(nm.sum())])
        lats[nm] = X2 @ c_lat; lons[nm] = X2 @ c_lon
    return list(zip(lons.tolist(), lats.tolist()))

def parse_svg_path(d):
    rings = []; current_ring = []
    tokens = re.findall(r'[MLCZSQTAHVmlczsqtahv]|[-+]?(?:\d+\.?\d*|\.\d+)(?:[eE][-+]?\d+)?', d)
    ...
    while i < len(tokens):
        ...
        if t in 'MLCZSQTAHVmlczsqtahv':
            cmd = t; i += 1; continue
        if cmd in ('M','m'):
            if current_ring and len(current_ring) > 2: rings.append(current_ring)
...
def extract_factions(svg_path):
    ...
    for path in root.iter('{http://www.w3.org/2000/svg}path'):
        ...
        if not label or 'junta' in label.lower(): continue
        d = path.get('d','')
        if not d: continue
    ...
    for (label,color),polys in faction_polys.items():
        try:
            ...
            features.append({"type":"Feature","properties":{
                "faction":canonical,"label_raw":label,"color":color,
                "area_deg2":round(merged.area,4),"is_junta":False
    ...
    try:
        ...
        if not junta_geom.is_empty:
            features.append({"type":"Feature","properties":{
                "faction":"Junta","label_raw":"Junta (reconstructed)","color":"#ebc0b3",
                "area_deg2":round(junta_geom.area,4),"is_junta":True
            },"geometry":mapping(junta_geom)})
    except Exception as e:
        print(f"  Warning reconstructing Junta: {e}")
    return features

def rasterize_priogrid(features, date_str):
    pg_cells = get_myanmar_priogrid_cells()
    geoms_primary = []; junta_geom = None
    for feat in features:
        try:
            g = shape(feat['geometry']); name = feat['properties']['faction']
            if name == 'Junta': junta_geom = g
            else: geoms_primary.append((name,g))
        except Exception: pass

    rows = []; n = len(pg_cells)
    for i,(gid,lat_c,lon_c) in enumerate(pg_cells):
        if i % 100 == 0: print(f"  Rasterizing... {i}/{n} cells", end='\r')
        sample_offsets = [-0.15, 0.0, 0.15]
        votes = []
        for dlat in sample_offsets:
            for dlon in sample_offsets:
                pt = Point(lon_c+dlon, lat_c+dlat)
                faction = 'Outside Myanmar'
                for name,geom in geoms_primary:
                    if geom.contains(pt): faction=name; break
                if faction=='Outside Myanmar' and junta_geom and junta_geom.contains(pt):
                    faction='Junta'
                votes.append(faction)
        real_votes = [v for v in votes if v != 'Outside Myanmar']
        faction = Counter(real_votes).most_common(1)[0][0] if real_votes else 'Outside Myanmar'
        if faction != 'Outside Myanmar':
            rows.append((gid, lat_c, lon_c, faction, date_str))
    print(f"  Rasterizing... done{' '*20}")
    return rows

def main():
    parser = argparse.ArgumentParser(description='Extract Myanmar SVG -> GeoJSON + PRIO-GRID CSV')
    ...
    if not os.access(out_dir, os.W_OK): out_dir = os.getcwd()
    ...
    rows = rasterize_priogrid(features, date_str)

    with open(csv_path,'w',newline='') as f:
        w = csv.writer(f)
        w.writerow(['priogrid_gid','lat','lon','faction','date'])
        w.writerows(rows)

    counts = Counter(r[3] for r in rows)
    print(f"\n  Grid CSV saved -> {csv_path}")
    print(f"  {len(rows):,} PRIO-GRID cells inside Myanmar")
    print(f"\n  {'Faction':<26} {'Cells':>6}  {'%Total':>8}")
    print(f"  {'-'*42}")
    for faction,n in counts.most_common():
        print(f"  {faction:<26} {n:>6}  {n/len(rows)*100:>7.1f}%")
